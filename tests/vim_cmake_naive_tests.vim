...
function! s:create_fake_cmake_script_with_build_error(root, args_output_path, exit_code) abort
  let l:bin_dir = s:path_join(a:root, 'fake-bin')
  call mkdir(l:bin_dir, 'p')
  let l:script_path = s:path_join(l:bin_dir, 'cmake')
  call writefile([
...
function! s:create_fake_cmake_script_with_long_build_output_error(root, args_output_path, output_line_count, exit_code) abort
  ...
  call mkdir(l:bin_dir, 'p')
  let l:script_path = s:path_join(l:bin_dir, 'cmake')
  let l:line_count = type(a:output_line_count) == v:t_number ? max([1, a:output_line_count]) : max([1, str2nr(a:output_line_count)])
  call writefile([
        \ '#!/bin/sh',
        \ 'printf "%s\n" "$@" > ' . shellescape(a:args_output_path),
        \ 'line_index=1',
        \ 'while [ "$line_index" -le ' . string(l:line_count) . ' ]; do',
        \ '  echo "build output line ${line_index}" >&2',
        \ '  line_index=$((line_index + 1))',
        \ 'done',
        \ 'echo "src/main.cpp:603:9: error: long output failure" >&2',
        \ 'exit ' . string(a:exit_code)
        \ ], l:script_path, 'b')
  call system('chmod +x ' . shellescape(l:script_path))
  return l:bin_dir
...
function! s:assert_terminal_buffer_name_not_result_title(terminal) abort
  let l:buffer_name = trim(get(a:terminal, 'buffer_name', ''))
  call assert_true(!empty(l:buffer_name), 'Expected terminal buffer name to be non-empty.')
  call assert_true(
        \ l:buffer_name !~# '^Success \[' && l:buffer_name !~# '^Failure \[',
        \ 'Expected terminal buffer name to remain unchanged by success/failure result titles.')
endfunction
...
function! s:wait_for_condition(timeout_ms, Condition) abort
  ...
  if exists('*wait')
    call wait(l:timeout_ms, {-> call(a:Condition, []) ? 1 : 0}, 10)
    return call(a:Condition, []) ? 1 : 0
  endif

  let l:elapsed = 0
  while l:elapsed < l:timeout_ms
    if call(a:Condition, [])
      return 1
    endif
    sleep 10m
...
function! s:wait_for_file(path, timeout_ms) abort
  call s:wait_for_condition(a:timeout_ms, {-> filereadable(a:path)})
  return filereadable(a:path)
endfunction

function! s:captured_build_terminal_text() abort
  let l:terminal = get(g:, 'vim_cmake_naive_test_last_build_terminal', {})
  return join(get(l:terminal, 'lines', []), "\n")
endfunction

function! s:wait_for_captured_build_terminal_output(fragment, timeout_ms) abort
  call s:wait_for_condition(a:timeout_ms, {-> stridx(s:captured_build_terminal_text(), a:fragment) >= 0})
  return get(g:, 'vim_cmake_naive_test_last_build_terminal', {})
endfunction

function! s:running_terminal_window_id() abort
...
function! s:wait_for_running_terminal_window(timeout_ms) abort
  call s:wait_for_condition(a:timeout_ms, {-> s:running_terminal_window_id() > 0})
  return s:running_terminal_window_id()
endfunction
...
function! s:wait_for_running_progress_message(terminal_title, timeout_ms) abort
  call s:wait_for_condition(a:timeout_ms, {-> s:has_running_progress_message(a:terminal_title)})
...
function! s:wait_for_statusline_value(expected_statusline, timeout_ms) abort
  call s:wait_for_condition(a:timeout_ms, {-> s:has_statusline_value(a:expected_statusline)})
...
function! s:active_running_cmake_command() abort
  ...
  return trim(l:running_command)
endfunction

function! s:has_running_cmake_terminal_job() abort
...
function! s:wait_for_running_cmake_command_release(timeout_ms) abort
  call s:wait_for_condition(
        \ a:timeout_ms,
        \ {-> empty(s:active_running_cmake_command()) && !s:has_running_cmake_terminal_job()})
  return empty(s:active_running_cmake_command()) && !s:has_running_cmake_terminal_job()
endfunction
...
function! s:wait_for_terminal_buffer_completion(buffer_number, timeout_ms) abort
  ...
  call s:wait_for_condition(
        \ a:timeout_ms,
        \ {-> !bufexists(a:buffer_number) || stridx(term_getstatus(a:buffer_number), 'running') < 0})
  if exists('*term_wait') && bufexists(a:buffer_number)
    call term_wait(a:buffer_number, 10)
  endif

  return !bufexists(a:buffer_number) || stridx(term_getstatus(a:buffer_number), 'running') < 0
endfunction
...
function! s:plugin_terminal_buffer_number_by_name(buffer_name) abort
  let l:latest_match = -1
  let l:buffer_name = trim(a:buffer_name)
  ...
  for l:buffer_info in getbufinfo()
    ...
    let l:terminal_title = trim(getbufvar(l:buffer_number, 'vim_cmake_naive_terminal_title', ''))
    if (!empty(l:buffer_name) && l:terminal_title ==# l:buffer_name)
          \ || bufname(l:buffer_number) ==# l:buffer_name
      let l:latest_match = l:buffer_number
      if exists('*term_getstatus') && stridx(term_getstatus(l:buffer_number), 'running') >= 0
        return l:buffer_number
      endif
    endif
  endfor

  return l:latest_match
endfunction
...
function! s:wait_for_plugin_terminal_buffer_name(buffer_name, timeout_ms) abort
  call s:wait_for_condition(
        \ a:timeout_ms,
        \ {-> exists('*term_getstatus')
        \    && s:plugin_terminal_buffer_number_by_name(a:buffer_name) > 0
        \    && stridx(term_getstatus(s:plugin_terminal_buffer_number_by_name(a:buffer_name)), 'running') >= 0})
  return s:plugin_terminal_buffer_number_by_name(a:buffer_name)
endfunction

function! s:quickfix_window_count() abort
...
function! s:wait_for_plugin_preview_output(fragment, timeout_ms) abort
  call s:wait_for_condition(a:timeout_ms, {
        \ -> stridx(
        \      join(s:terminal_buffer_non_empty_lines(s:plugin_preview_buffer_number()), "\n"),
        \      a:fragment) >= 0
        \ })
...
function! s:test_plugin_does_not_register_cmake_make_command() abort
  call assert_equal(0, exists(':CMakeMake'))
endfunction

function! s:test_plugin_startup_syncs_vimspector_variables_from_local_config() abort
...
function! s:test_plugin_startup_syncs_makeprg_when_enabled() abort
  let l:fixture = s:create_cmake_project_fixture()
  let l:initial_cwd = getcwd()
  let l:plugin_path = get(globpath(&runtimepath, 'plugin/vim_cmake_naive.vim', 1, 1), 0, '')
  let l:plugin_path = fnamemodify(l:plugin_path, ':p')
  let l:initial_loaded_plugin = get(g:, 'loaded_vim_cmake_naive', v:null)
  let l:initial_sync_makeprg = get(g:, 'vim_cmake_naive_sync_makeprg', v:null)
  let l:initial_make_errorformat = get(g:, 'vim_cmake_naive_make_errorformat', v:null)
  let l:initial_makeprg = &g:makeprg
  let l:initial_errorformat = &g:errorformat

  try
    call assert_true(!empty(l:plugin_path), 'Expected plugin/vim_cmake_naive.vim to exist in runtimepath.')
    call s:write_json(
          \ s:path_join(l:fixture.root, '.vim-cmake-naive-config.json'),
          \ {'output': 'out/build', 'preset': 'dev', 'target': 'my_target'})
    ...
    let l:expected_build_dir = s:path_join(l:expected_root, 'out/build/dev')
    call assert_true(stridx(&g:makeprg, '--build') >= 0)
    call assert_true(stridx(&g:makeprg, l:expected_build_dir) >= 0)
    call assert_true(stridx(&g:makeprg, '--parallel') >= 0)
    call assert_true(stridx(&g:makeprg, '--preset') >= 0)
    call assert_true(stridx(&g:makeprg, 'dev') >= 0)
    call assert_true(stridx(&g:makeprg, '--target') >= 0)
    call assert_true(stridx(&g:makeprg, 'my_target') >= 0)
    call assert_equal('%f:%l:%c: %m', &g:errorformat)
  finally
    if l:initial_loaded_plugin is v:null
      unlet! g:loaded_vim_cmake_naive
    else
    ...
    if l:initial_sync_makeprg is v:null
      unlet! g:vim_cmake_naive_sync_makeprg
    else
      let g:vim_cmake_naive_sync_makeprg = l:initial_sync_makeprg
    endif
    ...
    let &g:makeprg = l:initial_makeprg
    let &g:errorformat = l:initial_errorformat
    execute 'cd ' . fnameescape(l:initial_cwd)
    call delete(l:fixture.root, 'rf')
  endtry
endfunction
...
function! s:test_cmake_set_config_output_does_not_add_missing_vimspector_variables() abort
  let l:fixture = s:create_cmake_project_fixture()
  let l:initial_cwd = getcwd()
  ...
  try
    ...
    let l:variables = get(s:read_json(l:vimspector_path), 'variables', {})
    call assert_equal('keep', get(l:variables, 'UNRELATED', ''))
    call assert_false(has_key(l:variables, 'VIM_CMAKE_NAIVE_TARGET'))
    call assert_false(has_key(l:variables, 'VIM_CMAKE_NAIVE_OUTPUT'))
  finally
    execute 'cd ' . fnameescape(l:initial_cwd)
    call delete(l:fixture.root, 'rf')
  endtry
...
function! s:test_cmake_set_config_output_syncs_makeprg_when_enabled() abort
  let l:fixture = s:create_cmake_project_fixture()
  let l:initial_cwd = getcwd()
  let l:initial_sync_makeprg = get(g:, 'vim_cmake_naive_sync_makeprg', v:null)
  let l:initial_make_errorformat = get(g:, 'vim_cmake_naive_make_errorformat', v:null)
  ...
  try
    let l:config_path = s:path_join(l:fixture.root, '.vim-cmake-naive-config.json')
    call s:write_json(l:config_path, {'output': 'build', 'preset': 'dev', 'target': 'mylib'})
    ...
    call assert_true(stridx(&g:makeprg, 'dev') >= 0)
    call assert_true(stridx(&g:makeprg, '--target') >= 0)
    call assert_true(stridx(&g:makeprg, 'mylib') >= 0)
    call assert_equal('%f:%l:%c: %m', &g:errorformat)
  finally
    if l:initial_sync_makeprg is v:null
      unlet! g:vim_cmake_naive_sync_makeprg
...
function! s:test_cmake_set_commands_use_nearest_existing_local_config() abort
  ...
  try
    ...
    call s:write_json(l:root_vimspector_path, {
          ...
          \   'VIM_CMAKE_NAIVE_OUTPUT': 'root-output'
          \ }
          \ })
    call s:write_json(l:deep_vimspector_path, {
          \ 'variables': {
          \   'VIM_CMAKE_NAIVE_TARGET': 'deep-target',
          \   'VIM_CMAKE_NAIVE_OUTPUT': 'deep-output'
          \ }
          \ })
    ...
    let l:root_variables = get(s:read_json(l:root_vimspector_path), 'variables', {})
    let l:deep_variables = get(s:read_json(l:deep_vimspector_path), 'variables', {})
    ...
    call assert_equal('', get(l:root_variables, 'VIM_CMAKE_NAIVE_TARGET', 'MISSING'))
    call assert_equal('out/build/release', get(l:root_variables, 'VIM_CMAKE_NAIVE_OUTPUT', ''))
    call assert_equal('deep-target', get(l:deep_variables, 'VIM_CMAKE_NAIVE_TARGET', ''))
    call assert_equal('deep-output', get(l:deep_variables, 'VIM_CMAKE_NAIVE_OUTPUT', ''))
  finally
    execute 'cd ' . fnameescape(l:initial_cwd)
    call delete(l:fixture.root, 'rf')
  endtry
...
function! s:test_cmake_show_and_hide_preview_do_not_require_command_lock() abort
  if !has('unix')
    return
  endif
  ...
  try
    let l:args_path = s:path_join(l:fixture.root, 'cmake-build-preview-lock-args.txt')
    let l:bin_dir = s:path_join(l:fixture.root, 'fake-bin')
    let l:script_path = s:path_join(l:bin_dir, 'cmake')
    call mkdir(l:bin_dir, 'p')
    call writefile([
    ...
    call assert_true(l:terminal_buffer_number > 0, 'Expected build terminal buffer for preview lock test.')
    call assert_true(
          \ s:wait_for_terminal_buffer_completion(l:terminal_buffer_number, 3000),
          \ 'Expected build command to finish before preview lock test.')

    let g:vim_cmake_naive_test_forced_running_cmake_command = 'CMakeBuild'
    call assert_equal(0, s:plugin_preview_window_count(), 'Expected no preview windows before lock-bypassed preview commands.')

    call vim_cmake_naive#show_preview()
    call assert_true(
          \ bufwinnr(l:terminal_buffer_number) > 0,
          \ 'Expected CMakeShowPreview to run while command lock is active.')

    call vim_cmake_naive#hide_preview()
    call assert_equal(
          \ 0,
          \ s:plugin_preview_window_count(),
          \ 'Expected CMakeHidePreview to run while command lock is active.')
  finally
    if l:initial_forced_running_command is v:null
      unlet! g:vim_cmake_naive_test_forced_running_cmake_command
    else
...
function! s:test_cmake_generate_sets_running_terminal_name_on_subsequent_invocation() abort
  ...
  try
    ...
    call assert_true(
          ...
          \ 'Expected running generate statusline with warning highlight and command label.')
...
function! s:test_cmake_build_sets_running_terminal_name_with_preset_and_target() abort
  ...
  try
    ...
    call assert_true(
          \ s:wait_for_running_progress_message(l:running_buffer_name, 1000),
          \ 'Expected running build statusline with warning highlight and command label.')
    call assert_true(l:statusline_before !=# &g:statusline)
    call assert_equal(0, s:wait_for_running_terminal_window(200), 'Expected no visible build terminal window.')
    call assert_true(
          \ s:wait_for_terminal_buffer_completion(l:running_buffer_number, 3000),
...
function! s:test_cmake_build_opens_horizontal_terminal_with_command_output() abort
  ...
  try
    ...
    call assert_true(get(l:terminal, 'window_count', 0) >= 2)
    call assert_true(get(l:terminal, 'width', 0) > 0)
    call s:assert_terminal_buffer_name_not_result_title(l:terminal)
...
function! s:test_cmake_build_populates_quickfix_for_error_after_many_output_lines() abort
  if !has('unix')
    return
  endif
  ...
  try
    let l:args_path = s:path_join(l:fixture.root, 'cmake-build-args-quickfix-many-lines.txt')
    let l:bin_dir = s:create_fake_cmake_script_with_long_build_output_error(
          \ l:fixture.root,
          \ l:args_path,
          \ 550,
          \ 2)
    ...
    let l:terminal = s:wait_for_captured_build_terminal_output('build output line 550', 1000)
    call assert_equal(1, get(l:terminal, 'is_terminal', 0))
    call assert_true(s:wait_for_file(l:args_path, 1000), 'Expected fake cmake args file to be created.')
    ...
    let l:matches = filter(
          ...
          \ . ' && get(v:val, "col", 0) == 9'
          \ . ' && stridx(get(v:val, "text", ""), "long output failure") >= 0')
    call assert_true(!empty(l:matches), 'Expected quickfix list to include the long-output failure diagnostic entry.')
  finally
    let $PATH = l:initial_path
    if l:initial_capture_terminal is v:null
      unlet! g:vim_cmake_naive_test_capture_build_terminal
...
function! s:test_cmake_build_clears_quickfix_before_execution() abort
  if !has('unix')
    return
  endif
  ...
  try
    let l:args_path = s:path_join(l:fixture.root, 'cmake-build-args-quickfix-clear.txt')
    let l:bin_dir = s:create_fake_cmake_script(l:fixture.root, l:args_path, 0)
    let $PATH = l:bin_dir . ':' . l:initial_path
    let g:vim_cmake_naive_test_capture_build_terminal = 1
    call setqflist([{
    ...
    call assert_true(s:wait_for_file(l:args_path, 1000), 'Expected fake cmake args file to be created.')
    call assert_equal(0, len(getqflist()), 'Expected CMakeBuild to clear quickfix entries before execution.')
  finally
    let $PATH = l:initial_path
    if l:initial_capture_terminal is v:null
      unlet! g:vim_cmake_naive_test_capture_build_terminal
...
function! s:test_cmake_generate_reuses_build_output_window_in_async_mode() abort
  ...
  try
    ...
    call assert_true(
          \ s:wait_for_terminal_buffer_completion(l:first_running_buffer_number, 3000),
          \ 'Expected async build command to finish before generate starts.')
    call vim_cmake_naive#generate()
    ...
    call assert_true(
          \ s:wait_for_terminal_buffer_completion(l:second_running_buffer_number, 30000),
          \ 'Expected async generate command to complete.')
    let l:expected_root = s:normalized_path(l:fixture.root)
    call s:assert_cmake_build_parallel_args(
          \ s:read_non_empty_lines(l:build_args_path),
          \ s:path_join(l:expected_root, 'build'))
...
function! s:test_cmake_test_sets_running_terminal_name_with_preset() abort
  ...
  try
    ...
    call assert_true(
          \ s:wait_for_running_progress_message(l:running_buffer_name, 1000),
          \ 'Expected running test statusline with warning highlight and command label.')
    call assert_true(l:statusline_before !=# &g:statusline)
...
function! s:test_cmake_run_sets_running_terminal_name_with_preset_and_target() abort
  ...
  try
    ...
    call assert_true(
          \ s:wait_for_running_progress_message(l:running_buffer_name, 1000),
          \ 'Expected running run statusline with warning highlight and command label.')
    call assert_true(l:statusline_before !=# &g:statusline)
...
function! s:test_cmake_cancel_stops_running_build_terminal_job() abort
  if !has('unix')
    return
  endif
  ...
  try
    let l:state_path = s:path_join(l:fixture.root, 'build-cancel-state.txt')
    let l:bin_dir = s:path_join(l:fixture.root, 'fake-bin')
    call mkdir(l:bin_dir, 'p')
    let l:script_path = s:path_join(l:bin_dir, 'cmake')
    call writefile([
          ...
          \ 'sleep 20',
          \ 'echo "completed" >> ' . shellescape(l:state_path),
          \ 'exit 0'
          \ ], l:script_path, 'b')
    call system('chmod +x ' . shellescape(l:script_path))
    let $PATH = l:bin_dir . ':' . l:initial_path
    ...
    call vim_cmake_naive#cancel()

    call assert_true(
          \ s:wait_for_terminal_buffer_completion(l:running_buffer_number, 3000),
          \ 'Expected CMakeCancel to stop the running terminal job.')
    call assert_true(
          \ s:wait_for_running_cmake_command_release(3000),
          \ 'Expected command lock to release after CMakeCancel.')
    call vim_cmake_naive#cmake_config_default()
    call assert_true(
          \ filereadable(s:path_join(l:fixture.root, '.vim-cmake-naive-config.json')),
          \ 'Expected regular commands to run after CMakeCancel.')
  finally
    call s:wait_for_running_cmake_command_release(60000)
    let $PATH = l:initial_path
    if l:initial_capture_terminal is v:null
...
function! s:test_cmake_cancel_reports_missing_active_terminal_job() abort
  let l:fixture = s:create_cmake_project_fixture()
  let l:initial_cwd = getcwd()
  ...
  try
    ...
    let l:messages = execute('messages')
    call assert_true(stridx(l:messages, 'No active CMake terminal job to cancel.') >= 0)
  finally
    execute 'cd ' . fnameescape(l:initial_cwd)
    call delete(l:fixture.root, 'rf')
  endtry
...
function! s:test_cmake_show_and_hide_preview_toggle_latest_terminal_output() abort
  ...
  try
    ...
    let l:preview_window_id = win_getid(l:preview_window_number)
    let l:origin_window_id = win_getid()
    let l:messages_before_preview_noop = execute('messages')
    execute l:preview_window_number . 'wincmd w'
    call vim_cmake_naive#show_preview()
    call assert_equal(
          \ l:preview_window_id,
          \ win_getid(),
          \ 'Expected CMakeShowPreview to do nothing when invoked from preview terminal window.')
    call assert_equal(1, s:plugin_preview_window_count())
    call assert_equal(
          \ l:messages_before_preview_noop,
          \ execute('messages'),
          \ 'Expected CMakeShowPreview to avoid writing messages when invoked from preview terminal window.')
    if win_id2win(l:origin_window_id) > 0
      call win_gotoid(l:origin_window_id)
    endif
...
function! s:test_cmake_menu_popup_lists_commands_and_executes_selection() abort
  ...
  try
    ...
    call assert_equal(
          \ [' 1.   CMakeConfig', ' 2.   CMakeConfigDefault', ' 3.   CMakeSwitchPreset', ' 4.   CMakeSwitchBuild', ' 5.   CMakeSwitchTarget', ' 6.   CMakeGenerate', ' 7.   CMakeBuild', ' 8.   CMakeTest', ' 9.   CMakeRun', '10.   CMakeShowPreview', '11.   CMakeHidePreview', '12.   CMakeCancel', '13.   CMakeClose', '14.   CMakeInfo', '15.   CMakeMenu', '16.   CMakeMenuFull', '17.   CMakeConfigSetOutput'],
...
function! s:test_cmake_menu_executes_command_with_arguments() abort
  ...
  try
    ...
    execute 'cd ' . fnameescape(l:fixture.root)
    let g:vim_cmake_naive_test_use_popup_menu = 1
    let g:vim_cmake_naive_test_popup_menu_response = 17
...
function! s:test_cmake_switch_target_sets_selected_target() abort
  ...
  try
    ...
    call assert_equal('mylib', get(l:variables, 'VIM_CMAKE_NAIVE_TARGET', ''))
    call assert_equal('build/dev', get(l:variables, 'VIM_CMAKE_NAIVE_OUTPUT', ''))
  finally
    if l:initial_selection is v:null
      unlet! g:vim_cmake_naive_test_inputlist_response
    else
...
function! s:test_cmake_switch_target_syncs_makeprg_when_enabled() abort
  let l:fixture = s:create_cmake_project_fixture()
  let l:initial_cwd = getcwd()
  let l:initial_selection = get(g:, 'vim_cmake_naive_test_inputlist_response', v:null)
  let l:initial_use_popup = get(g:, 'vim_cmake_naive_test_use_popup_menu', v:null)
  ...
  try
    ...
    call mkdir(l:target_lib, 'p')
    call s:write_json(l:root_commands, s:fixture_entries())
    call s:write_json(s:path_join(l:target_app, 'compile_commands.json'), [{'file': '../src/main.cpp'}])
    call s:write_json(s:path_join(l:target_lib, 'compile_commands.json'), [{'file': '../lib/foo.cpp'}])
    ...
    execute 'silent CMakeSwitchTarget'
    call assert_true(stridx(&g:makeprg, '--build') >= 0)
    call assert_true(stridx(&g:makeprg, s:path_join(s:normalized_path(l:fixture.root), 'build/dev')) >= 0)
    call assert_true(stridx(&g:makeprg, '--target') < 0)
  finally
    if l:initial_selection is v:null
      unlet! g:vim_cmake_naive_test_inputlist_response
...
function! VimCMakeNaiveTestRunAll() abort
  ...
  call s:test_cmake_show_and_hide_preview_do_not_require_command_lock()
  ...
  call s:test_cmake_cancel_stops_running_build_terminal_job()
  call s:test_cmake_cancel_reports_missing_active_terminal_job()
